...
short is_present_in_etc(char* local_username, char* remote_subject) {
    dropbear_log(LOG_ERR, "is_present_in_etc enter");
    ...
    while (read_user_record(local_username_tmp, remote_subjects_tmp, authorization_file) >= 0) {
        if (strcmp(local_username, local_username_tmp) == 0) {
            dropbear_log(LOG_ERR, "Authorization record for user %s found, %s", local_username, remote_subjects_tmp);
            if (str_set_contains(remote_subjects_tmp, remote_subject, ':')) {
                dropbear_log(LOG_ERR, "Record contains remote subject %s. Authorizing.", remote_subject);
                fclose(authorization_file);
                return 1;
            }
            dropbear_log(LOG_ERR, "Record for user %s does not contain remote subject %s", local_username, remote_subject);
        }
    }
    fclose(authorization_file);

    dropbear_log(LOG_ERR, "Authorization record for user %s not found in /etc/", local_username);
}

short is_present_in_home(char* remote_subject) {
    dropbear_log(LOG_ERR, "is_present_in_home enter");
    ...
    if (str_set_contains(line, remote_subject, ':')) {
        dropbear_log(LOG_ERR, "Record contains remote subject %s found in home. Authorizing.", remote_subject);
        free(line);
    ...
    dropbear_log(LOG_ERR, "Authorization record not found in home");
    return 0;
}
...
short is_authorized(char* local_username, char* remote_subject) {
    dropbear_log(LOG_ERR, "Authorizing remote subject %s for access to local user %s", remote_subject, local_username);

    char authorized = 0;
    authorized = is_present_in_etc(local_username, remote_subject) || is_present_in_home(remote_subject);

    dropbear_log(LOG_ERR, "Authorization %d? record for user %s ", authorized, local_username);
    return authorized;
}
